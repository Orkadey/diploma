...
Данная работа посвящена разработке криминалистически правильных методов получения дампов оперативной памяти виртуальных машин. ~~Необходимо учитывать определенные требования к разрабатываемому инструменту:~~

* ~~изолированность инструмента от памяти исследуемой машины. Соблюдение данного условия обеспечивает целостность и корректность полученных данных;~~
* ~~достоверность результата. Инструмент должен учитывать особенности работы гипервизора и гостевой системы;~~
* ~~быстродействие. Получение дампов должно осуществляться в реальном времени.~~
...
1. Обзор методов получения дампов оперативной памяти;
2. Разработка семейства криминалистически правильных методов получения дампов памяти виртуальных машин; реализация эталонного метода;
3. Проведение вычислительного эксперимента для верификации разработанных методов.

## Глава 1

### Гипервизор

Гипервизор (монитор виртуальных машин) - это программное или аппаратное решение, позволяющее организовать на одном компьютере одновременную работу нескольких операционных систем.
...
### Платформа виртуализации QEMU/KVM
...
Эффективность подхода, основанного на использовании KVM, обеспечивается поддержкой технологии аппаратной виртуализации, которая добавляет режим выполнения, позволяющий поддерживать работу гостевых машин без необходимости предоставления им полного доступа к памяти и ресурсам. Для понимания принципов работы KVM предоставим концепцию Intel VT-x.

Intel VT-x можно рассматривать как "функцию, которая переключает обработку на гипервизор при обнаружением центральным процессорным устройством выполнения базовых команд (sensitive instructions)" **(формулировка?)**. Определены два типа базовых команд:

1. команды контроля ("control-sensitive instructions"), изменяющие состояние системных ресурсов;
2. команды, дейтсвующие в соответствии с состоянием ресурсов системы ("behaviour-sensitive instructions").

Команды первого типа, выполненые на виртуальной машине, задействуют операции на физической машине, в то время как команды второго типа показывают, что они были выполнены на виртуальной машине, поскольку результаты отличаются от тех, как если бы они были выполнены на физической машине.

Если программа пытается выполнить эти команды в гостевой системе без вмешательства, это вызовет проблемы на стороне гипервизора и гостевой системы. Отсюда возникает необходимость в детектировании процессором начала выполнения команды и в управлении гипервизором для выполнения этой инструкции на стороне программы.

CPU архитектуры x86 изначально не были предназначены для виртуализации, поэтому существовали команды, которые гостевая машина пыталась выполнить, но при этом они не могли быть выявлены посредством CPU, и, в результате, гипервизор не мог выполнить команды на стороне гостевой системы. Для решения данной проблемы была разработана технология Intel VT-x. Она добавляет новые режимы выполнения для процессора и переключения между этими режимами. Этими двумя режимами являются VMX (virtual machine execution) root operation и VMX non-root operation. Последний режим является гостевым режимом исполнения. Если производится попытка исполнения базовых команд, CPU обнаруживает попытку и переключает выполнение в режим VMX root operation, который является режимом выполнения для гипервизора. Это переключения, называемое "VM Exit", сигнализирует передачу контроля гипервизору, включая его для выполнения базовых инструкций на стороне гостевой системы [6].
