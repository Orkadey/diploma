...
Intel VT-x можно рассматривать как "функцию, которая переключает обработку на гипервизор для детектирования центральным процессорным устройством выполнения базовых команд (sensitive instructions)". Гипервизор является программой контроля для запуска и работы с виртуальными машинами (гостевыми системами) на физической машине. Определены два типа базовых команд:
...
Главная роль KVM заключается в обработке переключения "VM Exit" и "VM Entry". KVM реализован как модуль ядра Linux.

#### KVM и QEMU

KVM не может самостоятельно создавать виртуальные машины, для этого необходимо использовать QEMU - процесс пользовательского пространства.

> здесь про qemu

Поскольку QEMU является программным решением эмуляции интерпретирует и выполняет команды CPU по одной, что означает ограниченность быстродействия. Однако быстроедйствие QEMU можно значительно увеличить, если учитывать три условия:
...
Разработка KVM была основана на этой концепции. Применение этой идеи включает создание виртуальных машин, максимизируя использование ресурсов существующего решения с минимальными модификациями.

Последовательность выполнения QEMU/KVM показана на **рисунке x**. Сначала модуль ядра KVM создает файл /dev/kvm. Этот файл позволяет QEMU передавать различные запросы модулю ядра KVM для выполения функций гипервизора.

### Выделение памяти

В основном прцесс QEMU/KVM выполняется как большинство процессов операционной системы Linux. Память для процесса выделяется вызовом стандартной функции malloc() или mmap(). Например, если гостевая система создается с размером памяти равной 1ГиБ, QEMU/KVM вызовет malloc(1<<30), выделяя 1ГиБ виртуального адресного пространства хоста. Однако, как и в других случаях вызова функции malloc(), в действительности физическая память не выделяется до тех пор, пока не будет задействована. Как только гостевая операционная система начинает свою работу, она видит регион памяти выделенной функцией malloc() как свою физическую память. если ядро гостевой ситсемы обращается к тому, что находится, например, по адресу 0x0, оно увидит первую страницу выделенной памяти.

Изначально, всякий раз когда гостевая система изменяла свои таблици страниц (page table), было необходимо вмешательство хоста. Хост должен был проверять, что значения, которые гостевая система помещает в таблицы страниц являются корректными и обращались только к доступным областям памяти. Это осуществлялось при помощи двух механизмов.

Во-первых, набор таблиц страниц, используемых аппаратным обеспечением виртуализации существовало отдельно от таблиц страниц, которые гостевая система принимала за используемые. Сначала гостевая система проводила изменения в своих таблицах страниц, Далее, хост, обнаружая данные изменения, проверял их и изменял реальные таблицы страниц, используемые аппаратным обеспечением. Гостевой системе не позволено напрямую управлять таблицами страниц, используемых аппаратным обеспечением. Такая концепция называется теневые таблицы страниц и является часто используемым механизмом в виртуализации.
...
Приведенная техника работает надежно, однако имеет серьезные недостатки по критерию быстродействия. Единичное обращение к гостевой странице может занять до 25-и последовательных обращений, что означает высокие затраты по времени для выполнения оперций. Корень проблемы лежит в том, что каждое обращение в общем случае задействует таблицы страниц как гостевой системы так и хоста. Двухуровневая часть появляется из-зи необходимости гостевых таблиц страниц "самостоятельно" использовать таблицы страниц хоста. Таким образом, для хоста проверка и содержание теневых таблиц страниц могут оказаться затратными.
...
Проблема данного подхода заключается в том, что таблицы страниц хоста это то, что мы используем для принужденных действий. Например, если страница будет выгружена из памяти хоста, тогда необходимо скоординировать данное изменение в структурах EPT/NPT.

Программным решением является mmu_notifires в операционной системе Linux. Поскольку память QEMU/KVM является с точки зрения ядра частью памяти хоста, ядро может использовать подкачку страниц, перемещать области памяти и освобождать память. Однако, перед тем как страницы передаются обратно хосту для использования, гостевая машина получает сообщение о действиях хоста. Следовательно, гостевая система может устранить страницу из теневых таблиц страниц или структур EPT/NPT. После окончания необходимых оперций со стороны гостевой системы хост может свободно использовать страницы памяти.

Пример жизненного цикла страницы памяти:

Выделение памяти:

1. QEMU вызывает malloc() и выделяет память в виртуальном адресном пространстве для страницы без ссответствующей физической памяти;
...
3. ядро хоста выгружает страницу и убирает запись pte_t (предположим, что страница была использована только в одном месте);
