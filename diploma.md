...
Intel VT-x можно рассматривать как "функцию, которая переключает обработку на гипервизор при обнаружением центральным процессорным устройством выполнения базовых команд (sensitive instructions)" **(формулировка?)** , где гипервизор определяется как программа контроля для запуска и работы с виртуальными машинами (гостевыми системами) на физической машине. Определены два типа базовых команд:
...
2. команда может быть передана без модификации центральному процессорному устройству для выполнения в режиме VMX non-root opeation;
...
Последовательность выполнения QEMU/KVM показана на **рисунке x**. Сначала модуль ядра KVM создает файл /dev/kvm. Этот файл позволяет QEMU передавать различные запросы модулю ядра KVM для выполения функций гипервизора. Когда процесс QEMU запускается для создания гостевой системы, он повторно осуществляет системный вызов ioctl(), указывая файл /dev/kvm, передавая соответсвующий файловый дескриптор. Когда наступает время начала работы гостевой системы, QEMU снова вызывает ioctl() для отправки команды запуска гостевой системы модулю KVM. В свою очередь модуль ядра выполняет VM Entry и начинает работу гостевой системы. Далее, когда гостевая система собирается выполнить базовую команду, выполняется VM Exit, и KVM определяет причину выхода. Если для выполнения задачи ввода/вывода или другой задачи необходимо вмешательство QEMU, контроль передается QEMU процессу, и QEMU выполняет задачу. По окончании выполнения QEMU снова осуществляет вызов ioctl() и запрашивает KVM о продолжении работы гостевой системы. Такая последовательность выполнения повторяется на протяжении эмуляции виртуальной машины.

### Архитектура QEMU

Создание виртуальной машины начинается с запуска процесса qemu-kvm, для каждой виртуальной машины создается отдельный процесс, множество гостевых машин работают изолированно друг от друга. В момент выключения гостевой операционной системы процесс qemu-kvm завершается. Перезагрузка может быть произведена без завершения работы процесса.
