...
Гипервизор (монитор виртуальных машин) - это программное или аппаратное решение, позволяющее организовать на одном компьютере одновременную работу нескольких операционных систем. Среди множества гипервизоров выделим основные для различных операционных систем.

В операционных системах семейства Windows NT в настоящее время главную позицию занимает Hyper-V - гипервизор первого типа (bare-metal hypervisor). Hyper-V обеспечивает разграничение с помощью разделов - логических единиц, в которых работают гостевые системы. Каждый экземпляр гипервизора должен иметь по меньшей мере один родительский раздел, с запущенной Windows Server (2008 или позднее). Стек виртуализации запускается на родительском разделе и обладает прямым доступом к аппаратным устройствам. Затем родительский раздел порождает дочерние разделы, на которых и располагаются гостевые ОС. Дочерний раздел также может породить собственные дочерние разделы. Родительский раздел создает дочерние при помощи API, представленного в Hyper-V. Виртуальные утройства данного гипервизора поддерживают технологию Enlightened I/O, являющейся реализацией высокоуровненвых протоколов, таких как SCSI для прямого доступа к VMBus, что позволяет параллельно обрабатывать любые уровни эмуляции устройства.
...
В операционных системах семейства Linux большой популярностью пользуется QEMU (Quick Emulator), как одно из наиболее универсальных средств. Такие продукты, как VirtualBox, Xen имеют базовые составляющие QEMU [3]. В рамках данной работы важным преимуществом также является открытый исходный код эмулятора, а также поддерживаемость библиотекой libvirt, унифицирующей использование функционала различных мониторов виртуальных машин. В настоящее время QEMU может использоваться совместно с модулем KVM (Kernel-based Virtual Machine), что обеспечивает высокую производительность.
...
### 2.1 Методы получения дампов виртуальных машин
