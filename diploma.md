...
__Инструкции, которые могут повлиять на корректную работу гипервизора (sensitive-инструкции), должны быть перехвачены и контроль должен быть передан гипервизору. Это позволяет гарантировать полный контроль гипервизора над виртуализированными ресурсами. [ИСПРАВЛЕНИЕ]__
...
CPU архитектуры x86 изначально не были предназначены для виртуализации, поэтому существовали инструкции, которые гостевая машина пыталась выполнить, но при этом они не могли быть выявлены посредством CPU, и, в результате, гипервизор не мог выполнить инструкции на стороне гостевой системы. Для решения данной проблемы были разработаны технологии Intel VT-x и AMD-V. Они добавляют новые режимы выполнения для процессора и переключения между этими режимами. Этими двумя режимами являются VMX root operation и VMX non-root operation (VMX от "Virtual Machine eXecution"). Последний режим является гостевым режимом исполнения. __Если осуществляется попытка выполнения sensitive-инструкций в данном режиме, CPU обнаруживает попытку и переключает выполнение в режим VMX root operation, в котором работает гипервизор.[Не совсем понял, что надо]__ x __Если производится попытка исполнения базовых команд, CPU обнаруживает попытку и переключает выполнение в режим VMX root operation, который является режимом выполнения для гипервизора. [ПЕРЕФОРМУЛИРОВАТЬ!]__ Это переключение, называемое "VM Exit", сигнализирует передачу контроля гипервизору для выполнения sensitive-инструкций на стороне гостевой системы [6].
...
KVM не может самостоятельно создавать виртуальные машины. На хосте эмуляция устройств производится процессом пользовательского пространства QEMU. Это позволяет модулю ядра сфокусироваться на аспектах наиболее важных в плане производительности, в то время как __эмуляция устройств пользовательского пространства эмулирует аппаратное обеспечение (???)__ в изолированном процессе вне ядра системы. __[Предлагаю убрать, не помню о чем думал]__ Безопасность обеспечивается технологией sVirt, блокирующей процесс QEMU с помощью SELinux MAC (Mandatory Access Control) таким образом, чтобы ему предоставлялся доступ только к необходимиым файлам и ресурсам.

Поскольку QEMU является программным решением эмуляции, он интерпретирует и выполняет инструкции процессора по одной, что означает ограниченность быстродействия. Однако быстродействие можно значительно увеличить, если учитывать три условия:
...
__Данные условия порождают идею оптимизации работы гипервизора. Разработка KVM была основана на реализации этой концепции, при этом преследовалась цель внесения минимальных изменений в существующий монитор виртуальных машин при максимальном использовании его возможностей. __[переформулировано]__
...
__Шаг 0:__ __~~Модуль ядра~~ KVM (модуль ядра Linux или модуль ядра KVM???)__ создает файл `/dev/kvm`. __[просто KVM, т.е. создается из KVM]__ Этот файл позволяет QEMU передавать различные запросы модулю ядра KVM для выполения функций гипервизора. Когда процесс QEMU запускается для создания гостевой системы, он повторно осуществляет системный вызов `ioctl()`, указывая файл `/dev/kvm`, передавая соответсвующий файловый дескриптор. 

Шаг 1. Когда наступает время начала работы гостевой системы, QEMU снова вызывает `ioctl()` для отправки команды запуска гостевой системы модулю KVM. 

Шаг 2. В свою очередь модуль ядра выполняет VM Entry и начинает работу гостевой системы.

Шаг 3. Далее, когда гостевая система собирается выполнить базовую команду, выполняется VM Exit, и KVM определяет причину выхода. 

Шаг 4. Если для выполнения задачи ввода/вывода или другой задачи необходимо вмешательство QEMU, контроль передается QEMU процессу, и QEMU выполняет задачу. 
...
Полный список приведен в официальной документации. Кроме данных операций QEMU/KVM предоставляет возможность использования паравиртуализированных __[ИЗМЕНЕНО]__ драйверов балунинга памяти (англ. memory ballooning), сетевых соединений и хранилищ данных, которые увеличивают эффективность ввода/вывода и позволяют управлять количеством доступной оперативной памяти, выделяемой гостевой машине.
...
Разработанный метод требует небольших модификаций библиотеки libvirt, поскольку используемые функции, реализованные в данной библиотеке, имеют ограничения на размер буфера памяти, используемого при взаимодействии с QEMU/KVM. Данные ограничения связаны с тем, что API библиотеки libvirt адаптирован для удаленного взаимодействия с QEMU. Таким образом были произведены необходимые изменения, затрагивающие ограничения на размер сообщений передаваемых между QEMU и сообщяющимся приложением.

В качестве операционной системы физической машины использовался дистрибутив Ubuntu Linux. Операционной системой виртуальной машины, запущенной с помощью гипервизора QEMU/KVM являлась Windows 7 Ultimate x86-64 SP1, с выделенным объемом памяти равным 512МиБ.

В ходе получения дампа с помощью реализованного метода, работа гостевой системы не прекращалсь, при этом не было замечено значительных остановок работы виртуальной машины. Исходя из того, что приостановка гостевой операционной системы была сопряжена с приостановкой работы операционной системы хоста, было заключено, что данное явление было вызвано нехваткой ресурсов физической машины, поскольку состояние гостевой системы, исходя из архитектуры QEMU, не должно отражаться на состоянии операционной системы хоста. Повторное тестирование с использованием итерационного процесса и буфера меньшего размера и, соответственно, с меньшими затратами оперативной памяти подтвердило данную теорию - в работе виртуальной машины не наблюдалось замедлений. Таким образом, можно сделать вывод, что данный метод не требует остановки целевой системы.
...
Для проверки целостности считываемых блоков памяти применялся подсчет контрольных сумм. Контрольная сумма подсчитывалась при помощи алгоритма SHA5. Подсчет производился для считываемых блоков непосредственно перед их копированием и для результирующего содержимого буфера. Сравнение контрольных сумм показало идентичность каждого считываемого блока и содержимого буфера, как в случае итерационного процесса копирования, так и единичного считывания.

С целью дополнительной проверки полученных образов памяти на корректность использовался Volatility Framework - инструмент анализа дампов памяти. Данный инструмент проверяет дампы памяти на соответствие стандартам форматов и позволяет извлекать информацию о состоянии оперативной памяти из файлов образов. Применение Volatility позволило сравнить информацию об оперативной памяти и показало корректность полученного дампа.

## Заключение

В данной работе была рассмотрена архитектура платформы виртуализации QEMU/KVM и изучены основные подходы к получению дампов оперативной памяти виртуальных машин. На основе приобретенных знаний был разработан криминалистически правильный метод, не требующий остановки целевой системы.

В дальнейшем планируется разработать и реализовать метод получения дампа, основывающийся на создании дампа памяти процесса, при его обеспечении его непрерывной работы, поскольку данный подход помимо сохранения целостности памяти и неинвазивности обеспечивает универсальность решения задачи получения дампов памяти.
