...
Как следствие, сами виртуальные окружения и инфраструктура, лежащая в их основе, становятся потенциальной целью для атак злоумышленников [1]. Таким образом, растет необходимость в создании систем безопасности для противостояния внешним и внутренним угрозам в виртуализованных средах.
...
Наибольший интерес представляют методы, имеющие в своей основе принципы компьютерной криминалистики - дисциплины о раскрытии и расследовании преступлений, связанных с компьютерной информацией, о методах получения и исследовании доказательств, имеющих форму компьютерной информации и о применяемых для этого технических средствах [2]. Далее такие методы будем обобщать понятием «криминалистический подход». Ключевыми понятиями в определении компьютерной криминалистики являются получение и исследование доказательств, при этом необходимо придерживаться определенных требований, соответствующих целям данной дисплины:
...
Методы в той или иной степени могут удовлетворять требованиям криминалистического подхода к целостности и предотвращения «загрязнения» данных. По данному критерию можно выделить следующие группы методов:
...
Примером неинвазивного, криминалистически правильного метода, требующего остановки системы является стандартная функция гипервизора QEMU/KVM pmemsave. При её использовании гипервизор останавливает виртуальную машину, после чего снимает дамп оперативной памяти. Подробнее различные методы будут рассмотрены в главе 2.

### Цель работы

Данная работа посвящена разработке криминалистически правильных методов получения дампов оперативной памяти виртуальных машин, не требующих остановки целевых систем.
...
* тип 1, автономный (англ. bare-metal) — гипервизор, имеющий свои встроенные драйверы устройств, модели драйверов и планировщик, поэтому не зависящий от базовой ОС;
* тип 2, на основе базовой ОС (англ. hosted) — гипервизор, работающий в одном кольце с ядром основной ОС (кольцо 0), т. е. выполняющийся как процесс.
...
В операционных системах семейства Windows NT в настоящее время лидирующее место занимает гипервизор первого типа Hyper-V. Hyper-V разделяет ресурсы хоста между виртуальными машинами, используя концепцию *раздела* (англ. partition). Каждый экземпляр гипервизора должен иметь как минимум один *родительский раздел*, с запущенной операционной системой Windows Server (2008 или позднее). Сервер виртуализации исполняется в родительском разделе и имеет прямой доступ к аппаратным ресурсам хост-системы. Родительский раздел порождает дочерние разделы, в которых и исполняются гостевые ОС. Дочерний раздел также может породить собственные дочерние разделы. Родительский раздел создает дочерние при помощи API, представленного Hyper-V. Виртуальные утройства данного гипервизора поддерживают технологию Enlightened I/O, являющейся реализацией высокоуровневых протоколов таких как SCSI, для прямого доступа к VMBus, что позволяет параллельно взаимодействовать с любыми уровнями эмуляции устройств.
...
Intel VT-x и AMD-V можно рассматривать как функции центрального процессора, которые перехватывают особые инструкции, способные повлиять на корректную работу гипервизора, называемые sensitive-инструкциями (англ. sensitive instructions), и передают управление гипервизору. Существует два типа таких инструкций:
...
CPU архитектуры x86 изначально не были предназначены для виртуализации, поэтому существовали инструкции, которые гостевая машина пыталась выполнить, но при этом они не могли быть выявлены посредством CPU, и, в результате, гипервизор не мог выполнить инструкции на стороне гостевой системы. Для решения данной проблемы были разработаны технологии Intel VT-x и AMD-V. Они добавляют новые режимы выполнения для процессора и переключения между этими режимами. Этими двумя режимами являются VMX root operation и VMX non-root operation (VMX от англ. "Virtual Machine eXecution"). Последний режим является гостевым режимом исполнения. Если осуществляется попытка выполнения sensitive-инструкций в данном режиме, CPU обнаруживает попытку и переключает выполнение в режим VMX root operation, в котором работает гипервизор. Это переключение, называемое "VM Exit", сигнализирует передачу контроля гипервизору для выполнения sensitive-инструкций на стороне гостевой системы [6].
...
>Рис. 1. Переключение режимов выполнения инструкций, поддерживаемое технологиями аппаратной виртуализации

#### 1.3.2. Использование KVM в QEMU

KVM не может самостоятельно создавать виртуальные машины. На хосте эмуляция устройств производится процессом пользовательского пространства QEMU. Это позволяет модулю ядра сфокусироваться на аспектах наиболее важных в плане производительности. Безопасность обеспечивается технологией sVirt, блокирующей процесс QEMU с помощью SELinux MAC (Mandatory Access Control) таким образом, чтобы ему предоставлялся доступ только к необходимым программным и аппаратным ресурсам.

Поскольку QEMU является программным решением эмуляции, он интерпретирует и выполняет инструкции процессора по одной, что означает ограниченность быстродействия. Однако быстродействие можно значительно увеличить, если учитывать следующие условия:

1. Инструкция может быть напрямую выполнена на CPU;
2. Инструкция может быть передана без модификации центральному процессору для выполнения в режиме VMX non-root opeation;
3. Инструкция, которая не может быть выполнена напрямую, может быть идентифицирована и передана QEMU для обработки.

Данные условия подводят к идеи оптимизации работы гипервизора. Проект KVM был основан на реализации этой концепции, при этом преследовалась цель внесения минимальных изменений в существующий монитор виртуальных машин при максимальном использовании его возможностей.

Последовательность выполнения QEMU/KVM показана на рисунке 2. 

__Шаг 0.__ KVM создает файл `/dev/kvm`. Этот файл позволяет QEMU передавать различные запросы модулю ядра KVM для выполения функций гипервизора. Когда процесс QEMU запускается для создания гостевой системы, он повторно осуществляет системный вызов `ioctl()`, указывая файл `/dev/kvm`, передавая соответсвующий файловый дескриптор. 

__Шаг 1.__ Когда наступает время начала работы гостевой системы, QEMU снова вызывает `ioctl()` для отправки команды запуска гостевой системы модулю KVM. 

__Шаг 2.__ В свою очередь модуль ядра выполняет VM Entry и начинает работу гостевой системы.

__Шаг 3.__ Далее, когда гостевая система собирается выполнить базовую команду, выполняется VM Exit, и KVM определяет причину выхода. 

__Шаг 4.__ Если для выполнения задачи ввода/вывода или другой задачи необходимо вмешательство QEMU, контроль передается QEMU процессу, и QEMU выполняет задачу. 

По окончании выполнения QEMU снова осуществляет вызов `ioctl()` и запрашивает KVM о продолжении работы гостевой системы (шаг 1). Такая последовательность выполнения повторяется на протяжении эмуляции виртуальной машины.

![KVM QEMU](https://github.com/Orkadey/diploma/blob/master/pictures/KVM_QEMU_and_KVM.png)

>Рис.2. Алгоритм оптимизации работы гипервизора с использованием KVM
...
1. никакой другой код QEMU не выполняется в одно и то же время, благодаря чему отпадает необходимость в синхронизации. Функции обратного вызова выполняются последовательно и атомарно по отношению к другому коду.
...
Выполнение гостевого кода приводит к передаче контроля управления выполнения кода с хоста гостевой системе. Пока поток исполняет код гостевой системы, он не может одновременно находиться в главном цикле обработки событий. В таком случае гостевая система теоретически может сохранять контроль неограниченное время, что означает её полную остановку. Для устранения данной проблемы используются сигналы. Сигналы прерывают работу текущего потока и вызывает функцию обработки сигналов. Это позволяет QEMU выйти, перенаправить контроль управления и вернуться в главный цикл обработки событий. Результатом этого является то, что новые события могут быть обнаружены не сразу, если QEMU выполняет гостевой код. Чаще всего QEMU в конечном счете обрабатывает события, но эта задержка сама по себе является проблемой в плане быстродействия. С этой целью таймеры, завершения операций ввода/вывода и уведомления рабочих потоков ядра QEMU используют сигналы для обеспечения немедленного выполнения цикла обработки событий.
...
>Рис.3. Общая архитектура платформы виртуализации QEMU/KVM
...
1. Механизма *теневых таблиц страниц* (англ. shadow page tables). Теневые таблицы страниц используются гипервизором для отслеживания гостевых таблиц страниц. Гостевая система не должна иметь доступ к таблицам страниц хоста, поскольку это означало бы наличие контроля гостевой системы над ресурсами хоста. Поэтому во время выполнения кода гостевой системы гипервизор скрывает от виртуальной машины таблицы страниц, переводящие гостевой виртуальный адрес в физический адрес хоста. При использовании гостевой системой области памяти, не имеющей соответсвующей записи в теневых таблицах страниц, возникает `page fault`, перехватываемый и обрабатываемый гипервизором, в результате чего в теневые таблицы страниц заносятся соответсвующие записи.
...
1. VT-d (Intel Virtualization Technology for Directed I/O). VT-d является механизмом трансляции адресов для устройств ввода/вывода (IOMMU). Он предоставляет таблицы перевода адресов для DMA перемещения данных устройств, а также обеспечивает маршрутизацию и изолированность аппаратных прерываний.
...
* миграция виртуальных машин - перемещение гостевой виртуальной машины с одного QEMU процесса на другой.

Полный список приведен в официальной документации. Кроме данных операций QEMU/KVM предоставляет возможность использования паравиртуализированных драйверов балунинга памяти (англ. memory ballooning), сетевых соединений и хранилищ данных, которые увеличивают эффективность ввода/вывода и позволяют управлять количеством доступной оперативной памяти, выделяемой гостевой машине.
...
Большая часть администрирования происходит с использованием API библиотеки libvirt, в особенности инструмента virsh, который предоставляет операции управления гостевой системой и хостом. Ту же цель имеет графический инструмент virt-manager. Сторонние инструменты такие как OpenStack могут быть использованы для высокоуровнего управления датацентрами и, чаще всего, интегрируемы с libvirt.

Далее более детально рассмотрим особенности организации памяти в QEMU, а также функционал, существенно влияющий на структуру памяти гостевой системы, поскольку данные аспекты могут непосредственным образом повлиять на решение поставленных в данной работе задач.
...
* Контейнер (container). Контейнер включает в себя другие регионы памяти, каждый из которых располагается по разным адресам. Контейнеры используются для группирования нескольких регионов в один, например, PCI BAR может быть представлен в виде объединения регионов RAM и MMIO. Регионы составляющие контейнер обычно не перекрываются. В некоторых случаях удобным оказывается использование перекрывающихся регионов, например, для контролера, который может перекрывать субрегионы RAM регионами MMIO или ROM;
...
![Memory_struct](https://github.com/Orkadey/diploma/blob/master/pictures/mem_example.png)

>Рис.4. Пример структуры памяти
...
Оперативная память виртуальных машин гипервизоров второго типа, каковым является QEMU/KVM, представляет из себя выделенный блок памяти хоста, который располагается внутри процесса QEMU. Следовательно, для получения дампов памяти виртуальны машин можно применять методы, получающие часть оперативной памяти хоста, выделенной соответсвующему процессу QEMU, либо непосредственно блок памяти, представляющий оперативную память гостевой системы.
...
Исходя из архитектуры QEMU/KVM, описанной в первой главе, можно заключить, что предложенный метод является криминалистически правильным, неинвазивным и не требующим остановки целевой системы.
...
* выявление преимуществ и недостатков реализованного метода в сравнении со стандартными методами получения дампов, главным образом, стандвртными методами гипервизора QEMU/KVM.
...
Для реализации разработанного метода было принято решение внести изменения в код библиотеки libvirt, поскольку используемые в ней по умолчанию функции и параметры имеют неподходящие ограничения на размер буфера памяти для чтения памяти гипервизоров и связаны с тем, что API libvirt адаптирован для удаленного взаимодействия.

В качестве операционной системы физической машины использовался дистрибутив GNU/Linux Ubuntu 14.10. Операционной системой виртуальной машины, запущенной с помощью гипервизора QEMU/KVM являлась Windows 7 Ultimate x86-64 SP1, с выделенным объемом памяти равным 512 МиБ.

В ходе получения дампа с помощью реализованного метода, работа гостевой системы не прекращалсь, при этом не было замечено значительных остановок работы виртуальной машины. Говоря о незначительных по времени приостановках гостевой операционной системы, можно сказать, что они были связаны с остановкой работы хост-системы в целом и вызваны нехваткой ресурсов физической машины (поскольку состояние гостевой системы, исходя из архитектуры QEMU, не может отражаться на состоянии операционной системы хоста). Повторное тестирование с использованием итерационного процесса и буфера меньшего размера и, соответственно, с меньшими затратами оперативной памяти подтвердило данную теорию - в работе виртуальной машины остановок не наблюдалось. Таким образом, можно сделать вывод, что данный метод не требует остановки целевой системы.

Неинвазивность напрямую следует из применяемого алгоритма работы. Необходимо показать криминалистическую правильность метода, проверив целостность получаемого дампа памяти.

Для проверки целостности считываемых блоков памяти применялся подсчет контрольных сумм. Контрольная сумма подсчитывалась при помощи алгоритма SHA256. Подсчет производился для считываемых блоков непосредственно перед их копированием и для результирующего содержимого буфера. Сравнение контрольных сумм показало идентичность каждого считываемого блока и содержимого буфера, как в случае итерационного процесса копирования, так и единичного считывания.

С целью дополнительной проверки полученных образов памяти на корректность использовался Volatility Framework - инструмент анализа дампов памяти. Данный инструмент проверяет дампы памяти на соответствие стандартам форматов дампов памяти и позволяет извлекать информацию о состоянии оперативной памяти из файлов образов. Применение Volatility позволило сравнить информацию об оперативной памяти и показало корректность полученного дампа.
...
В дальнейшем планируется разработать и реализовать метод получения дампа, основывающийся на создании дампа памяти процесса, при обеспечении его непрерывной работы, поскольку данный подход, помимо сохранения целостности памяти и неинвазивности, обеспечивает универсальность решения задачи получения дампов памяти.
...
3. QEMU Internals. http://qemu.weilnetz.de/qemu-tech.html#QEMU-Internals
...
5. Kernel Virtual Machine. http://www.linux-kvm.org/page/Memory

6. Yasunori Goto. Kernel-based Virtual Machine Technology // FUJITSU Sci. Tech. J., Vol. 47, No. 3, July 2011. P. 362–368
...
8. Checkpoint/Retore In Userspace. http://criu.org/Main_Page
